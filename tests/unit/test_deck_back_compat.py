...
def test_meter_to_mm_conversion_for_cabin_trunk() -> None:
    """Legacy cabin_trunk_length (spec 033: 2.4 m) maps to CabinTrunkParameters.length in mm."""
    legacy = DeckParameters()  # cabin_trunk_length=2.40
    sp = legacy.to_superstructure_parameters()
    assert sp.cabin_trunk.length == pytest.approx(2400.0)
...
def test_legacy_deck_parameters_remains_frozen_and_compatible() -> None:
    ...
    assert p.cabin_trunk_length == 2.40  # spec 033 orientation fix
    assert p.cabin_trunk_fwd_offset == 1.60  # spec 033
    assert p.cabin_trunk_width == 2.20
    assert p.cabin_trunk_height == 0.95  # spec 033 proportion correction (low greenhouse)
